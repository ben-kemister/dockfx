...
public class DockPane extends StackPane implements EventHandler<DockEvent> {
  ...
  /**
   * Whether or not this dock pane allows the docking of dock nodes from external sources (i.e.,
   * other dock panes).
   */
  ...
  public DockPane() {
    ...
    dockPosButtons = FXCollections.observableArrayList(dockCenter, dockTop, dockRight, dockBottom,
        dockLeft, dockTopRoot, dockRightRoot, dockBottomRoot, dockLeftRoot);
  ...
  /**
   * Indicates whether or not the dock pane is in exclusive mode (the default). In exclusive mode,
   * the dock pane ignores dock nodes dragged from other dock panes, and dock nodes dragged from
   * this dock pane are ignored by other dock panes.
   * 
  ...
  /**
   * Tries to write the layout preferences of this {@link DockPane} in the given file path.
   * 
   * @param outputStream
   */
  ...
  /**
   * Tries to write the layout preferences of this {@link DockPane} in the given
   * {@link OutputStream}.
   * 
   * @param outputStream
   */
  public void storePreference(OutputStream outputStream) {
  ...
  /**
   * Returns the contents (layout, panes etc.) of this {@link DockPane}, ready for storage.
   * 
   * @return
   */
  private HashMap<String, ContentHolder> getContentsForPreferences() {
    ...
    contents.put("_FloatingNodes",
        new ContentHolder("_FloatingNodes", ContentHolder.Type.Collection));
    ...
    for (int i = 0; i < floatingNodes.size(); i++) {
      ...
      floatingNode.addProperty("Size",
          new Double[] {floatingNodes.get(i).getLayoutBounds().getWidth(),
  ...
  /**
   * Tries to load the layout preferences of this {@link DockPane} from the given file path.
   * 
   * @param filePath
   */
  ...
  /**
   * Tries to load the layout preferences of this {@link DockPane} from the given {@link InputStream}.
   * 
   * @param inputStream
   * @param delayOpenHandler
   */
  ...
  private void applyPane(Map<String, ContentHolder> contents, ContentPane root,
  ...
  private Node buildPane(ContentPane parent, ContentHolder holder,
      HashMap<String, DockNode> dockNodes, DelayOpenHandler delayOpenHandler) {
    Node pane = null;
    if (holder.getType().equals(ContentHolder.Type.SplitPane)) {
      ...
      for (Object item : holder.getChildren()) {
        if (item instanceof String) {
          ...
          splitPane.getItems()
              .add(buildPane(splitPane, (ContentHolder) item, dockNodes, delayOpenHandler));
        }
      }
      if (parent != null)
